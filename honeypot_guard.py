...
def install(core):
    ...
    async def setup(interaction: discord.Interaction):
        ...
        await _ensure(DB)
        await DB.execute("INSERT INTO honeypot_settings (guild_id,channel_id,enabled,triggered) VALUES (%s,%s,1,0) ON CONFLICT (guild_id) DO UPDATE SET channel_id=EXCLUDED.channel_id,enabled=1,triggered=0,triggered_at=NULL", interaction.guild.id, interaction.channel.id)
