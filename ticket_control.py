...
Keeps the legacy bot intact and adds a self-contained ticket system:
- /티켓질문 켜기|끄기|상태
- /티켓질문내용 <질문>
- /티켓설정 <카테고리> [지원역할]
- /티켓패널
...
def _admin(interaction: discord.Interaction) -> bool:
    ...
    return bool(isinstance(member, discord.Member) and (member.guild_permissions.manage_guild or member.guild_permissions.administrator))


async def _settings(core, guild_id: int):
    row = await core.DB.fetchone(
        "SELECT ticket_category_id, ticket_role_id, ticket_message, COALESCE(ticket_questions_enabled, 1) AS ticket_questions_enabled, COALESCE(ticket_question, ticket_message) AS ticket_question FROM guild_settings WHERE guild_id = %s",
        guild_id,
    )
    if row:
        return row
    await core.DB.execute(
        "INSERT INTO guild_settings (guild_id, ticket_questions_enabled, ticket_message, ticket_question) VALUES (%s, 1, %s, %s) ON CONFLICT (guild_id) DO NOTHING",
        guild_id, "무엇을 도와드릴까요?", "무엇을 도와드릴까요?",
    )
...
async def _open_ticket(core, interaction: discord.Interaction, answer: Optional[str] = None):
    ...
    existing = await core.DB.fetchone("SELECT channel_id FROM ticket_logs WHERE guild_id = %s AND owner_id = %s", guild.id, interaction.user.id)
    if existing:
        channel = guild.get_channel(int(existing.get("channel_id")))
        if channel:
            return await interaction.response.send_message(f"이미 열린 티켓이 있습니다: {channel.mention}", ephemeral=True)
        await core.DB.execute("DELETE FROM ticket_logs WHERE channel_id = %s", int(existing.get("channel_id")))
    ...
    try:
        channel = await guild.create_text_channel(name=f"ticket-{safe_name}", category=category, overwrites=overwrites, topic=f"DinoBot ticket | owner={interaction.user.id}", reason=f"DinoBot ticket opened by {interaction.user}")
    except discord.Forbidden:
        return await interaction.response.send_message("티켓 채널을 만들 권한이 없습니다. 봇에게 채널 관리 권한을 주세요.", ephemeral=True)
    ...
    await core.DB.execute("INSERT INTO ticket_logs (channel_id, guild_id, owner_id, opened_at) VALUES (%s,%s,%s,%s)", channel.id, guild.id, interaction.user.id, core.now_kst_str())
    embed = discord.Embed(title="🎫 DinoBot 티켓", description="담당자가 확인할 때까지 이 채널에 문의 내용을 남겨주세요.", color=discord.Color.blurple())
    ...
    await channel.send(content=(f"{interaction.user.mention} {staff_role.mention if staff_role else ''}".strip()), embed=embed, view=CloseTicketView(core))
...
class TicketQuestionModal(discord.ui.Modal, title="문의 내용"):
    question = discord.ui.TextInput(label="무엇을 도와드릴까요?", placeholder="문의 내용을 자세히 적어주세요.", style=discord.TextStyle.paragraph, required=True, max_length=1000)
...
class CloseTicketView(discord.ui.View):
    ...
    async def close(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        row = await self.core.DB.fetchone("SELECT owner_id FROM ticket_logs WHERE channel_id = %s AND guild_id = %s", interaction.channel.id, interaction.guild.id)
        if not row:
        ...
        is_staff = isinstance(interaction.user, discord.Member) and (interaction.user.guild_permissions.manage_channels or interaction.user.guild_permissions.administrator)
...
def install(core) -> None:
    """Register persistent views and slash commands without touching legacy_main.py."""
    bot = core.bot
    ...
    async def migrate_once():
        ...
        if migration_done:
            return
        try:
            await core.DB.execute("ALTER TABLE guild_settings ADD COLUMN IF NOT EXISTS ticket_questions_enabled INTEGER DEFAULT 1")
            await core.DB.execute("ALTER TABLE guild_settings ADD COLUMN IF NOT EXISTS ticket_question TEXT DEFAULT '무엇을 도와드릴까요?'")
            migration_done = True
        except Exception:
    ...
    if bot.tree.get_command("티켓질문") is None:
        ...
        async def enable(interaction: discord.Interaction):
            await core.DB.execute("INSERT INTO guild_settings (guild_id, ticket_questions_enabled) VALUES (%s, 1) ON CONFLICT (guild_id) DO UPDATE SET ticket_questions_enabled = 1", interaction.guild.id)
        ...
        async def disable(interaction: discord.Interaction):
            await core.DB.execute("INSERT INTO guild_settings (guild_id, ticket_questions_enabled) VALUES (%s, 0) ON CONFLICT (guild_id) DO UPDATE SET ticket_questions_enabled = 0", interaction.guild.id)
        ...
        async def status(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(f"🎫 티켓 질문: {'🟢 켜짐' if enabled else '🔴 꺼짐'}\n질문 내용: {cfg.get('ticket_question') if cfg else '설정 없음'}", ephemeral=True)
    ...
    if bot.tree.get_command("티켓질문내용") is None:
        ...
        async def set_question(interaction: discord.Interaction, 질문: str):
            ...
            await core.DB.execute("INSERT INTO guild_settings (guild_id, ticket_message, ticket_question) VALUES (%s,%s,%s) ON CONFLICT (guild_id) DO UPDATE SET ticket_message = EXCLUDED.ticket_message, ticket_question = EXCLUDED.ticket_question", interaction.guild.id, question, question)
    ...
    if bot.tree.get_command("티켓설정") is None:
        ...
        async def configure(interaction: discord.Interaction, 카테고리: discord.CategoryChannel, 지원역할: Optional[discord.Role] = None):
            await core.DB.execute("INSERT INTO guild_settings (guild_id, ticket_category_id, ticket_role_id) VALUES (%s,%s,%s) ON CONFLICT (guild_id) DO UPDATE SET ticket_category_id = EXCLUDED.ticket_category_id, ticket_role_id = EXCLUDED.ticket_role_id", interaction.guild.id, 카테고리.id, 지원역할.id if 지원역할 else None)
            await interaction.response.send_message(f"✅ 티켓 설정 완료\n카테고리: {카테고리.mention}\n담당 역할: {지원역할.mention if 지원역할 else '없음'}", ephemeral=True)

    if bot.tree.get_command("티켓패널") is None:
        @bot.tree.command(name="티켓패널", description="현재 채널에 DinoBot 티켓 패널을 전송합니다.")
        @app_commands.check(_admin)
        async def panel(interaction: discord.Interaction):
            embed = discord.Embed(title="🎫 문의 티켓", description="문의가 필요하다면 아래 버튼을 눌러 티켓을 열어주세요.\n질문 기능이 켜져 있으면 먼저 문의 내용을 입력받습니다.", color=discord.Color.blurple())
