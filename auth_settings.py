...
The interactive Discord /인증설정 command is owned exclusively by
verification_controls.py. This module only provides dashboard APIs and the
separate /인증설정알림 command, preventing duplicate slash commands.
"""
...
def install(core):
    ...
    async def ensure():
        try:
            ...
            log.exception("verification settings migration failed")
    ...
    async def notify(guild: discord.Guild, title: str, description: str):
        row = await DB.fetchone(
            "SELECT verification_log_channel_id,log_channel_id FROM guild_settings WHERE guild_id=%s",
            guild.id,
        ) or {}
    ...
    async def verification_announce(interaction: discord.Interaction):
        ...
        captcha = bool(int(row.get("verification_captcha_enabled") or 0))
        ip = bool(int(row.get("verification_ip_collection_enabled") or 0))
        ok = await notify(
            interaction.guild,
            "🔐 인증 정책 안내",
            f"**CAPTCHA:** {'사용' if captcha else '사용 안 함'}\n**IP 수집:** {'사용' if ip else '사용 안 함'}\n\nIP 수집을 사용하는 경우 서버 운영자는 수집 목적과 보관 정책을 별도로 안내해야 합니다.",
        )
        await interaction.response.send_message(
            "✅ Discord 로그 채널에 게시했습니다." if ok else "⚠️ 인증 로그 채널이 설정되지 않았거나 메시지를 보낼 수 없습니다.",
            ephemeral=True,
        )

    # Deliberately no /인증설정 registration here. verification_controls.py is
    # the single owner of that command and its interactive save-and-run UI.
    bot.tree.add_command(verification_announce)
    ...
    async def auth_page(request: Request, guild_id: int):
        ...
        logs = await DB.fetchall(
            "SELECT user_id,event,captcha_passed,ip_address,created_at FROM verification_logs WHERE guild_id=%s ORDER BY id DESC LIMIT 50",
            guild_id,
        )
        rows = "".join(
            f"<tr><td>{html.escape(str(x.get('user_id')))}</td><td>{html.escape(str(x.get('event')))}</td><td>{'통과' if x.get('captcha_passed') else '해당 없음'}</td><td>{html.escape(str(x.get('ip_address') or '미수집'))}</td><td>{html.escape(str(x.get('created_at')))}</td></tr>"
            for x in logs
        ) or "<tr><td colspan='5'>아직 인증 로그가 없습니다.</td></tr>"
        checked_c = "checked" if captcha else ""
        checked_i = "checked" if ip else ""
    ...
    async def save_auth(request: Request, guild_id: int):
        ...
        captcha = 1 if str(form.get("captcha", "0")) == "1" else 0
        ip = 1 if str(form.get("ip", "0")) == "1" else 0
        raw = str(form.get("channel_id", "")).strip()
        cid = int(raw) if raw.isdigit() else 0
        await DB.execute(
            "INSERT INTO guild_settings (guild_id,verification_captcha_enabled,verification_ip_collection_enabled,verification_log_channel_id) VALUES (%s,%s,%s,%s) ON CONFLICT (guild_id) DO UPDATE SET verification_captcha_enabled=EXCLUDED.verification_captcha_enabled,verification_ip_collection_enabled=EXCLUDED.verification_ip_collection_enabled,verification_log_channel_id=EXCLUDED.verification_log_channel_id",
            guild_id, captcha, ip, cid or None,
        )
        ok = await notify(
            guild,
            "🔐 인증 설정 변경",
            f"<@{uid}>님이 인증 정책을 변경했습니다.\n\n**CAPTCHA:** {'사용' if captcha else '사용 안 함'}\n**IP 수집:** {'사용' if ip else '사용 안 함'}\n**로그 채널:** {'<#'+str(cid)+'>' if cid else '미설정'}",
        )
