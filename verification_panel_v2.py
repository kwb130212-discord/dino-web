...
def install(core):
    ...
    # Remove every previous registration of this command. Do not mutate
    # Command.callback: discord.py exposes it as a read-only property.
    while bot.tree.get_command("인증설정") is not None:
        bot.tree.remove_command("인증설정")

    def admin_ok(i):
        return bool(i.guild and isinstance(i.user, discord.Member)
                    and i.user.guild_permissions.administrator)
    ...
    class Panel(discord.ui.View):
        ...
        def build(self):
            self.clear_items()
            self.add_item(Toggle(self, "🧩 CAPTCHA", "captcha", 0))
            self.add_item(Toggle(self, "🌐 IP 로그", "ip", 0))
            self.add_item(Channel(self))
            self.add_item(Role(self))
            self.add_item(Design(self))
            self.add_item(Save(self))
        ...
        async def redraw(self, i):
            self.build()
            await i.response.edit_message(embed=self.settings_embed(), view=self)

    class Toggle(discord.ui.Button):
        def __init__(self, p, label, attr, row):
            self.p, self.attr = p, attr
            enabled = bool(getattr(p, attr))
        ...
        async def callback(self, i):
            if not admin_ok(i):
                return await i.response.send_message("❌ 서버 관리자 권한이 필요합니다.", ephemeral=True)
            setattr(self.p, self.attr, not getattr(self.p, self.attr))
            await self.p.redraw(i)
    ...
    class Channel(discord.ui.ChannelSelect):
        ...
        async def callback(self, i):
            if not admin_ok(i):
                return await i.response.send_message("❌ 관리자 권한이 필요합니다.", ephemeral=True)
            self.p.log_channel_id = self.values[0].id
            await self.p.redraw(i)
    ...
    class Role(discord.ui.RoleSelect):
        ...
        async def callback(self, i):
            ...
            if role.is_default() or role.managed or (me and role >= me.top_role):
                return await i.response.send_message("❌ 봇의 최고 역할보다 아래의 역할만 선택할 수 있습니다.", ephemeral=True)
            self.p.role_id = role.id
            await self.p.redraw(i)

    class Design(discord.ui.Button):
        def __init__(self, p):
            self.p = p
            super().__init__(label="패널 디자인 / 이미지 URL", emoji="🖼️", style=discord.ButtonStyle.secondary, row=3)
        async def callback(self, i):
            if not admin_ok(i):
                return await i.response.send_message("❌ 관리자 권한이 필요합니다.", ephemeral=True)
    ...
    class Save(discord.ui.Button):
        ...
        async def callback(self, i):
            ...
            try:
                ...
                if role and (role.is_default() or role.managed or (g.me and role >= g.me.top_role)):
                ...
                parts = []
                if self.p.top_text:
                    parts.append(self.p.top_text)
                if self.p.bottom_text:
                    parts.append(self.p.bottom_text)
                ...
                view = VerificationView(g.id, self.p.button_text or "인증하기")
    ...
    async def command_callback(i: discord.Interaction):
        ...
        if not await core.is_server_admin(i.user, i.guild.id):
            return await i.response.send_message("❌ 서버 관리자 권한이 필요합니다.", ephemeral=True)
        p = Panel(i.guild.id, await _state(DB, i.guild.id))
        await i.response.send_message(embed=p.settings_embed(), view=p, ephemeral=True)

    bot.tree.add_command(app_commands.Command(
        name="인증설정",
        description="인증 관련 설정과 이미지 URL 인증패널을 한 곳에서 관리합니다.",
