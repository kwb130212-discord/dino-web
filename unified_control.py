# -*- coding: utf-8 -*-
"""Discord-first controls and dashboard channel browser.

Verification collection controls intentionally live only in the canonical
/인증설정 control center. Keeping a second /인증수집데이터 command here caused
command duplication and split configuration state.
"""
...
class ChannelSelect(discord.ui.Select):
    def __init__(self, guild: discord.Guild):
        ...
        options = [
            discord.SelectOption(
                label=c.name[:100],
                value=str(c.id),
                emoji="🔊" if isinstance(c, discord.VoiceChannel) else "💬",
            )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"선택한 채널: {channel.mention if channel else '없음'}",
            ephemeral=True,
        )
...
def install(core) -> None:
    ...
    if not getattr(bot, "_unified_control_installed", False):
        ...
        async def channel_list(interaction: discord.Interaction):
            ...
            other = [
                ...
                if getattr(c, "category_id", None) is None and not isinstance(c, discord.CategoryChannel)
            ]
            if other:
                lines.append("**📁 카테고리 없음**")
                lines.extend(
                    f"　{'🔊' if isinstance(c, discord.VoiceChannel) else '💬'} {c.mention}"
                    for c in other
                )
            ...
            await interaction.response.send_message(
                ...
                view=ChannelView(interaction.guild),
                ephemeral=True,
            )
        ...
        async def join_leave(
            ...
            await save(
                ...
                welcome_enabled=int(입장),
                leave_enabled=int(퇴장),
                welcome_image_url=입장사진,
                leave_image_url=퇴장사진,
            )
            e = discord.Embed(title="👋 입퇴장 설정 저장 완료", color=discord.Color.green())
            e.add_field(name="채널", value=채널.mention)
            e.add_field(name="입장", value="ON" if 입장 else "OFF")
            e.add_field(name="퇴장", value="ON" if 퇴장 else "OFF")
            if 입장사진:
        ...
        async def server_settings(interaction: discord.Interaction):
            ...
            e.add_field(
                name="🔐 인증",
                value=f"CAPTCHA {'ON' if row.get('verification_captcha_enabled') else 'OFF'}\nIP 기록 {'ON' if row.get('verification_ip_collection_enabled') else 'OFF'}",
                inline=True,
            )
            e.add_field(
                name="👋 입퇴장",
                value=f"입장 {'ON' if row.get('welcome_enabled') else 'OFF'}\n퇴장 {'ON' if row.get('leave_enabled') else 'OFF'}",
                inline=True,
            )
            await interaction.response.send_message(embed=e, ephemeral=True)
    ...
    async def channel_api(request: Request, guild_id: int):
        ...
        uncategorized = [
            {"id": c.id, "name": c.name, "type": c.__class__.__name__}
            for c in guild.channels
            if getattr(c, "category_id", None) is None and not isinstance(c, discord.CategoryChannel)
        ]
        return JSONResponse({
    ...
    async def channel_page(request: Request, guild_id: int):
        ...
        for cat in data["categories"]:
            parts.append(
                "<section><h2>📁 %s</h2>%s</section>" % (
                    ...
                    "".join(
                        "<div class='ch'>💬 %s <code>%s</code></div>"
                        % (html.escape(c["name"]), c["id"])
        ...
        if data["uncategorized"]:
            parts.append(
                "<section><h2>📁 카테고리 없음</h2>%s</section>"
                % "".join(
                    "<div class='ch'>💬 %s <code>%s</code></div>"
                    % (html.escape(c["name"]), c["id"])
                    for c in data["uncategorized"]
                )
            )
        body = (
            "<!doctype html><html lang='ko'><meta charset='utf-8'>"
            "<meta name='viewport' content='width=device-width,initial-scale=1'>"
            "<title>DinoBot 채널</title>"
            "<style>body{margin:0;background:#070b14;color:#f7f9fc;"
            "font:14px system-ui,Pretendard,sans-serif}main{max-width:900px;margin:auto;"
            "padding:25px 15px}section{background:#0f1728;border:1px solid #24324b;"
            "border-radius:14px;padding:14px;margin:12px 0}.ch{padding:10px;"
            "border-top:1px solid #1d2940}code{float:right;color:#91a0b7}a{color:#9da7ff}"
            "</style><main><a href='/dashboard/server/%d'>← 서버 설정</a>"
            "<h1>📋 %s 채널 목록</h1><p>봇이 현재 접근 가능한 채널입니다.</p>%s</main></html>"
            % (guild_id, html.escape(data["guild_name"]), "".join(parts))
        )
