...
def _decode_state(state: str, max_age: int = 600) -> dict | None:
    try:
...
def _verify_state(state: str, max_age: int = 600) -> bool:
    return _decode_state(state, max_age) is not None
...
def install(core) -> None:
    ...
    os.environ["VERIFY_REDIRECT_URI"] = redirect_uri
    ...
    async def dashboard_callback(request: Request):
        ...
        if not payload:
            return page('<div class="wrap"><main class="card"><div class="brand">DinoBot</div><h1 class="title">OAuth State 오류</h1><p class="desc">인증 요청이 만료되었거나 유효하지 않습니다.</p><a class="btn" href="/dashboard/login">다시 로그인</a></main></div>')
        ...
        if purpose == "verification" and guild_id:
            try:
                ...
                log.exception("verification completion failed guild=%s user=%s", guild_id, uid)
                role_ok, role_message = False, "인증 데이터 저장 또는 역할 부여 중 오류가 발생했습니다."
