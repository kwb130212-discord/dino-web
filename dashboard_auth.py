...
def install(core) -> None:
    ...
    # One source of truth. Render's REDIRECT_URI is the value that must match
    # the Discord Developer Portal character-for-character.
    ...
    def nav() -> str:
        ...
        <a href="/">로비</a><a href="{invite}" target="_blank" rel="noopener">+ 디노봇 추가하기</a>
        <a href="/dashboard">웹 대시보드</a><a href="{support}" target="_blank" rel="noopener">서포트 서버</a>
        </nav></header>'''

    def page(body: str, title: str = "DinoBot") -> HTMLResponse:
        css = """
        :root{color-scheme:dark;--bg:#070a10;--line:#202938;--text:#f5f7fb;--muted:#8e9aac;--accent:#6572ff}
        *{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(900px 500px at 50% -10%,#20294b 0%,transparent 65%),var(--bg);color:var(--text);font-family:Inter,Pretendard,system-ui,sans-serif}
        .nav{height:64px;display:flex;align-items:center;justify-content:space-between;padding:0 28px;border-bottom:1px solid var(--line);background:rgba(7,10,16,.78);backdrop-filter:blur(16px);position:fixed;top:0;left:0;right:0;z-index:10}.logo{color:var(--text);font-weight:850;text-decoration:none}.nav nav{display:flex;gap:6px}.nav nav a{color:#aeb8c9;text-decoration:none;font-size:13px;padding:9px 12px;border-radius:9px}.nav nav a:hover{background:#151c29;color:#fff}
        .wrap{min-height:100vh;display:grid;place-items:center;padding:96px 24px 24px}.card{width:min(410px,100%);padding:38px 32px;border:1px solid var(--line);background:rgba(13,18,27,.92);border-radius:24px;box-shadow:0 24px 80px #0008;text-align:center}.avatar{width:64px;height:64px;border-radius:50%;object-fit:cover;border:1px solid #33405a;margin-bottom:16px}.brand{font-size:13px;letter-spacing:.16em;text-transform:uppercase;color:#9ca8bc;margin-bottom:28px}.title{font-size:25px;font-weight:800;margin:0 0 9px}.desc{color:var(--muted);line-height:1.7;margin:0 0 25px}.btn{display:flex;align-items:center;justify-content:center;width:100%;height:48px;border:0;border-radius:12px;background:var(--accent);color:#fff;text-decoration:none;font-weight:750}.small{font-size:12px;color:#68758a;margin-top:18px}.ok{font-size:15px;color:#8ff0bb;margin:5px 0 20px}
    ...
    async def dashboard_login(request: Request):
        ...
        body = '<div class="wrap"><main class="card"><div class="brand">DinoBot Control Center</div><h1 class="title">대시보드 로그인</h1><p class="desc">Discord 계정으로 로그인한 뒤<br>관리 가능한 서버를 확인할 수 있습니다.</p><a class="btn" href="' + html.escape(auth_url, quote=True) + '">Discord로 계속하기</a><div class="small">DinoBot · Secure Dashboard</div></main></div>'
    ...
    async def dashboard_callback(request: Request):
        ...
        if error:
            log.warning("OAuth 인증 취소/실패: error=%s", error)
