...
Keeps authentication separate from the management UI so the login/callback
pages can be redesigned without touching server-management code.
"""
...
def install(core) -> None:
    ...
    def page(body: str, title: str = "DinoBot") -> HTMLResponse:
        ...
        *{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(900px 500px at 50% -10%,#20294b 0%,transparent 65%),var(--bg);color:var(--text);font-family:Inter,Pretendard,system-ui,sans-serif}
        .wrap{min-height:100vh;display:grid;place-items:center;padding:24px}.card{width:min(410px,100%);padding:38px 32px;border:1px solid var(--line);background:rgba(13,18,27,.92);border-radius:24px;box-shadow:0 24px 80px #0008;text-align:center}
        ...
        """
        return HTMLResponse(f"<!doctype html><html lang='ko'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>{title}</title><style>{css}</style></head><body>{body}</body></html>")
    ...
    async def dashboard_login(request: Request):
        ...
        body = '<div class="wrap"><main class="card"><div class="brand">DinoBot Control Center</div><h1 class="title">대시보드 로그인</h1><p class="desc">Discord 계정으로 로그인한 뒤<br>관리 권한이 있는 서버만 표시됩니다.</p><a class="btn" href="' + auth_url + '">Discord로 계속하기</a><div class="small">DinoBot · Secure Dashboard</div></main></div>'
    ...
    async def dashboard_callback(request: Request):
        ...
        request.session["avatar_url"] = (
            f"https://cdn.discordapp.com/avatars/{uid}/{me['avatar']}.png?size=128" if me.get("avatar") else
        ...
        body = f'<div class="wrap"><main class="card"><div class="brand">DinoBot Control Center</div><img class="avatar" src="{avatar}" alt=""><h1 class="title">{safe_name}</h1><div class="ok">인증완료</div><p class="desc">감사합니다!<br>잠시 후 Control Center로 이동합니다.</p><a class="btn" href="/dashboard">대시보드로 이동</a><div class="small">인증이 완료되었습니다.</div></main></div><script>setTimeout(()=>location.href="/dashboard",1200)</script>'
    ...
    # Put the auth routes before older feature modules. This makes the auth UI
    # deterministic even when older dashboard routes remain installed.
